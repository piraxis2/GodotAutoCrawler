...
```text
addons/save_game/                 # 순수 SaveGame core (domain-free)
...
addons/save_game_world_state/     # SaveGame ↔ WorldState 통합 adapter (선택)
  world_state_save_section.gd     # class_name WorldStateSaveSection extends SaveSection
  tests/
```

> 두 폴더는 interim 위치다. 장기 목표는 `addons/world_core/save_game/`와
> `addons/world_core/save_game_world_state/`이며, 패키징 이동은 별도 Task로 수행한다(ADR-013).

## 설치 (호스트)

1. **폴더 복사**: `addons/save_game/`(core)와, WorldState를 저장한다면 `addons/save_game_world_state/`도 복사.
...
3. **autoload 수동 등록**: `SaveGameManager`를 autoload로 등록한다. 권장 이름은 `SaveGame`이며, autoload 이름은
   ...
   | `SaveFlowFacade`(선택) | `res://addons/save_game/save_flow.gd` |
...
```text
godot --headless --path <project> res://addons/save_game/tests/sg001_step1_core_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg001_step1_static_guard_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg001_step2_slot_store_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg001_step4_backup_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg002_step1_save_flow_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg002_step1_static_guard_test.tscn
godot --headless --path <project> res://addons/save_game/tests/sg003_step2_host_flow_test.tscn
godot --headless --path <project> res://addons/save_game_world_state/tests/sg001_step3_world_state_section_test.tscn
godot --headless --path <project> res://addons/save_game_world_state/tests/sg002_step2_save_flow_world_state_test.tscn
```
