...
`addons/dialogtool/` 폴더 하나만 복사하면 대화 에디터, 런타임, World State 코어/condition,
...
```text
addons/dialogtool/
  dialoguetool.gd            # EditorPlugin 진입점
  plugin.cfg
  Editor/ Node/ Resource/ RunTime/ UI/ debugger_plugin/   # DialogueTool 본체
  world_state/               # World State 하위모듈 (코어 + condition)
  ...
  examples/                  # 게임이 자기 것으로 교체할 예제 리소스
    world_state_schema_example.tres   # out-of-box 부팅용 예제 Schema (6-key)
    affinity_ge_10.tres               # 예제 ConditionSet (actor.example.affinity >= 10)
    sample_dialogues/
      sample_world_state_dialogue.tres  # state_condition 분기 + state_add 데모 대화
```

## 설치 (새 프로젝트)

1. **폴더 복사**: 대상 프로젝트에 `addons/dialogtool/` 전체를 복사한다.
...
3. **플러그인 활성화**: Project Settings → Plugins → "DialogueTool" Enable.
   ...
   > 낸다. 이 경우 아래 autoload 목록에 `DialogueToolUtil="*res://addons/dialogtool/dialoguetool_util.gd"`를
...
4. **런타임 autoload 수동 등록 (순서 권장)**: Project Settings → Autoload에 아래를 추가한다.
   ...
   | 이름 | 경로 |
   | --- | --- |
   | `DialogueManager` | `res://addons/dialogtool/RunTime/dialogue_manager.gd` |
   | `WorldState` | `res://addons/dialogtool/world_state/world_state_store.tscn` |
   | `WorldStateRuntime` | `res://addons/dialogtool/world_state/world_state_runtime.gd` |
   ...
   ```ini
   [autoload]
   DialogueToolUtil="*res://addons/dialogtool/dialoguetool_util.gd"
   DialogueManager="*res://addons/dialogtool/RunTime/dialogue_manager.gd"
   WorldState="*res://addons/dialogtool/world_state/world_state_store.tscn"
   WorldStateRuntime="*res://addons/dialogtool/world_state/world_state_runtime.gd"
   ```

설치 직후 `WorldState`는 `examples/world_state_schema_example.tres`로 부팅돼 바로 동작하고,
`examples/sample_dialogues/sample_world_state_dialogue.tres`를 에디터에서 열어 구조를 확인할 수 있다.

## 게임 Schema로 교체

`examples/world_state_schema_example.tres`는 **예제**다. 실제 게임 상태 키는 호스트가 소유한다.
...
예제는 `examples/world_state_schema_example.tres`(6-key) 참고.
...
1. 해당 폴더를 `addons/dialogtool/world_state/`로 **이동**한다(복사 금지 — 같은 `class_name`이 두 곳에
