...
func _capture(node: DialogueNode) -> void:
	auto_load_idx = node.autoload_option.get_selected_id()
	property_idx = node.property_option.get_selected_id()
	autoload_name = node.autoload_option.get_item_text(auto_load_idx)
	property_name = node.property_option.get_item_text(property_idx)
