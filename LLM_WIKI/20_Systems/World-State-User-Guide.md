...
현재 `world_state_store.tscn`은 유효한 `world_state_schema.tres`를 참조하며 `WorldState` autoload로
등록돼 있다. 게임 부팅 직후 Store는 ready지만, gameplay를 시작하려면 `WorldStateRuntime`에서
...
현재 게임용 Schema는 다음 경로에 있다.

```text
res://Assets/Script/gds/world_state/world_state_schema.tres
```
...
Dialogue에는 State Condition 노드가 있다(DT-008). `WorldState`(또는 임의 read provider)를
`DialogueManager.play`에 넘기면 `state_condition` Data 노드가 그 provider로 `ConditionSet`을 평가해
Branch와 조건부 Choice를 제어한다. State Read/Set 노드는 아직 없다.
...
```text
DialogueManager.play(resource, WorldState)
  -> DialogueUI.play(resource, WorldState)
...
다만 현재 DialoguePlayer에는 mutation provider가 주입되지 않으며 이를 소비하는 State Effect 노드도
없다. 게임 코드에서는 Store를 직접 호출하고, Dialogue 조건/Effect 연결은 후속 Task에서 추가한다.
...
```text
res://Assets/Script/gds/world_state/tests/dt005_step6_integration_test.tscn
res://Assets/Script/gds/world_state/tests/dt006_step5_integration_test.tscn
```
