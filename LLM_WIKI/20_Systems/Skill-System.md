...
- `SkillDefinition : Resource`: v0 필드 `Id`, `DisplayName`, `Range`, `ActCost`, `WindupCost`, `ManaCost`, `AnimationName`, `TargetSide`, `TargetSelectorDefault`, `AmmoResetScope`, `Ammo`, `Effects`. `MasterCost = max(ActCost + WindupCost, 1)`. `IsValidV0`은 `Ammo >= 0`과 `AmmoResetScope` enum 정의를 함께 검증한다.
...
- 사거리 내 무대상이면 `SelectTarget`이 `null`을 반환한다. SK-002 이후 `TurnAction_Skill.Init`은 이 경우 커밋 이전 무소모 `ActionState.Failure`를 내며, BT Selector fallback을 탄다.
...
- `SkillDefinition.ManaCost`: SK-002 이후 `TurnAction_Skill.Init`은 `ManaCost > 0`일 때 먼저 `Mana.CanAfford`로 지불 가능성만 확인한다. 부족하면 상태 변경/소모 없이 `SkillState.Failed`를 세우고, `Action`이 `ActionState.Failure`로 반환한다. 실제 `Mana.TrySpend`는 대상 락온(커밋) 뒤 ammo와 함께 수행한다. `ManaCost == 0`(slash/magicbolt)은 게이트 없음 → 기존 baseline 보존.
...
스킬별 **고정 보장 사용 횟수** `ammo`. 플레이어가 발수를 배분하지 않고 `SkillDefinition`에 정의된 수량으로 자동 충전한다. WC식 배분형 탄수는 채택하지 않는다.

- **데이터 모델:** `SkillDefinition.AmmoResetScope`(`Unlimited`/`Battle`/`Expedition`) + `Ammo`(int). 기본값 `Unlimited`+`0`이라 기존 `.tres`는 무제한으로 로드된다(backward-compatible). remaining은 Definition에 **저장하지 않는다**.
...
- `Assets/Scenes/Character/SkillCaster.tscn`([[SK-003-Skill-Based-Character-Scene]])은 최신 데이터 스킬 캐릭터 샘플이다. 기존 Soldier 기반 캐릭터 씬의 상태/애니메이션/BT 구조를 재사용하되 legacy `TurnAction_ChainLightning` 대신 `TurnAction_Skill`을 사용한다.
- BT에는 `staff_chainlightning`(Battle ammo 1, windup 1, mana 30)을 먼저 두고, 실패/소진 시 `staff_magicbolt`(Unlimited, mana 5)로 fallback하도록 같은 root selector 아래에 배치했다.
- 이 씬은 production `battle_field.tscn`에는 아직 배치하지 않았다. encounter 투입은 전투 밸런스와 CB-001 결정론 baseline을 함께 다뤄야 한다.
## Compatibility Seam
