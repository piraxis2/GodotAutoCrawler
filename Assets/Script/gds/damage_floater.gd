...
func display(damage: int, position: Vector2, is_critical: bool):
	...
	tween.tween_property(number, "position", number.position + Vector2(3,-5), 0.25).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BOUNCE)
	tween.tween_property(number, "scale", Vector2.ZERO, 0.1).set_ease(Tween.EASE_IN).set_delay(0.1)
